import Link from "next/link";
import Image from "next/image";
import logo from "@/../public/brand/logo.png";
import { FaFacebook, FaInstagram, FaTwitter } from "react-icons/fa6";

const Footer = () => {
  return (
    <footer className="p-top bg-bg-surface">
      <div className="container-fluid grid sm:grid-cols-3 lg:grid-cols-5 gap-6 pb-8">
        {/* Brand */}
        <div className="sm:col-span-3 lg:col-span-2">
          <Image src={logo} alt="logo" className="h-12 w-auto mb-2" />
          <p className="text-text-secondary">
            Your go-to platform for booking movie tickets quickly and easily.
            Discover the latest releases, grab the best seats, and enjoy the
            show!
          </p>
        </div>

        {/* More */}
        <div>
          <h2 className="text-2xl font-medium mb-4 text-text-primary">More</h2>
          <ul className="text-sm space-y-2 text-text-secondary">
            <li>
              <Link
                href="/how-it-works"
                className="hover:text-primary transition-colors"
              >
                How it Works
              </Link>
            </li>
            <li>
              <Link
                href="/contact"
                className="hover:text-primary transition-colors"
              >
                Contact
              </Link>
            </li>
            <li>
              <Link
                href="/privacy-policy"
                className="hover:text-primary transition-colors"
              >
                Privacy & policy
              </Link>
            </li>
          </ul>
        </div>

        {/* Contact */}
        <div>
          <h2 className="text-2xl font-medium mb-4 text-text-primary">
            Contact Us
          </h2>
          <ul className="text-sm space-y-2 text-text-secondary">
            <li>
              <a
                href="tel:+8801647211326"
                className="hover:text-primary transition-colors"
              >
                (+880) 1647211326
              </a>
            </li>
            <li>
              <a
                href="tel:+8801319438030"
                className="hover:text-primary transition-colors"
              >
                01319438030
              </a>
            </li>
            <li>
              <a
                href="mailto:ajrabbyk72@gmail.com"
                className="hover:text-primary transition-colors"
              >
                ajrabbyk72@gmail.com
              </a>
            </li>
          </ul>
        </div>

        {/* Social */}
        <div>
          <h2 className="text-2xl font-medium mb-4 text-text-primary">
            Follow Us
          </h2>
          <ul className="text-sm space-y-2 text-text-secondary">
            <li>
              <a
                href="https://www.facebook.com/rabbykhan04"
                className="flex items-center gap-2 hover:text-primary transition-colors"
                target="_blank"
              >
                <FaFacebook className="text-xl" />
                <span>Facebook</span>
              </a>
            </li>
            <li>
              <a
                href="https://www.instagram.com/rabbykhan04"
                className="flex items-center gap-2 hover:text-primary transition-colors"
                target="_blank"
              >
                <FaInstagram className="text-xl" />
                <span>Instagram</span>
              </a>
            </li>
            <li>
              <a
                href="https://x.com/RabbyKh17969501"
                className="flex items-center gap-2 hover:text-primary transition-colors"
                target="_blank"
              >
                <FaTwitter className="text-xl" />
                <span>Twitter</span>
              </a>
            </li>
          </ul>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="bg-primary/10 py-3">
        <div className="container-fluid flex items-center justify-center sm:justify-between flex-wrap gap-2 text-text-secondary">
          <p>
            &copy; All Rights Reserved By <span className="italic">me</span>
          </p>
          <p>
            Design & Developed by <span className="italic">me</span>
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
